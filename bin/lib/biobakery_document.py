# -*- coding: utf-8 -*-
"""Self-contained document generation for the bioBakery vis/stats reports.

Lifted from anadama2.document (v0.10.0) so the Nextflow pipeline does not
depend on the AnADAMA2 workflow framework. The class is unchanged apart from
the three couplings to the rest of anadama2, which are replaced below:

  * ``from .helpers import sh``  -> a local sh() built on subprocess
  * ``from . import Task``       -> dropped; no AnADAMA Task objects are ever
                                    passed to this class from Nextflow
  * ``Document.create(task)``    -> the task argument was already unused

Everything else -- the plotting methods the .pmd templates and
biobakery_workflows.visualizations call -- is byte-for-byte the original, so
reports render identically to `biobakery_workflows vis|stats`.

anadama2 is distributed under the MIT license; see LICENSE-anadama2 alongside
this file.
"""
import os
import tempfile
...
def sh(command, log_command=True, **kwargs):
    """ Run a shell command, replacing anadama2.helpers.sh.

        Returns a callable so the two call sites in create() keep their
        original sh(...)() form.
    """

    def actually_sh(task=None):
        if log_command:
            logging.getLogger(__name__).info("Executing: %s", command)
        kwargs["shell"] = True
...
class PweaveDocument(Document):
    ...
    def __init__(self, templates=None, depends=None, targets=None, vars=None, table_of_contents=None):
        ...
        if self.depends and self.data_folder and os.path.isdir(self.data_folder):
            depends_files = list(filter(None, self.depends))
    ...
    def create(self, task):
        ...
        if temp_template.endswith(".py"): 
            ...
            with open(intermediate_template) as f:
                md = f.read()
            import re
            md = re.sub(r'(!\[[^\]]*\]\([^\)\n]+)\n([^\n]*\))', r'\1\2', md)
            with open(intermediate_template, "w") as f:
                f.write(md)   
    ...
    def show_pcoa(self, sample_names, feature_names, data, title, sample_types="samples", feature_types="species",
        ...
        if metadata:
            if metadata_type == 'con':
                # ax=subplot is a deliberate divergence from anadama2 0.10.0,
                # which calls pyplot.colorbar(scalarmappaple) with no axes. The
                # ScalarMappable is built standalone here, never attached to an
                # axes, so matplotlib >= 3.6 cannot infer where to steal space
                # for the colorbar and raises "Unable to determine Axes to steal
                # space for Colorbar". That kills the chunk, so a report with
                # any continuous metadata loses every continuous PCoA plot.
                pyplot.colorbar(scalarmappaple, ax=subplot)
