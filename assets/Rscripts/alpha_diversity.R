...
# VENDORED from biobakery_workflows 3.2 (Rscripts/alpha_diversity.R).
#
# One change, marked inline below: the scatterplot branch called `labs("")`,
# passing an unnamed argument. ggplot2 4.x builds its labels as an S7 object
# whose validator rejects unnamed labels ("<ggplot2::labels> object is invalid:
# every label must be named"), so the script aborted on the first continuous
# metadata variable and the vis report lost every alpha diversity scatterplot.
# The call was a no-op even under ggplot2 3.x, so dropping it changes nothing
# about the plot -- it only stops the abort.
#
# biobakery_bootstrap.get_rscript() prefers this copy over the installed one.
# Delete this file once upstream fixes the call and the pinned R stack moves.
...
alpha_plot <- function(alpha_meta, category)
{
  ...
  if (is.character(meta[[category]]) | is.factor(meta[[category]]) ) {
    ...
    p <- ggplot(data = meta, aes(x = !!sym(category), invsimpson)) + 
      ...
      # upstream has `labs("") +` here; see the note at the top of this file
      xlab(category) + ylab(paste(index, "Diversity")) + theme_bw(base_size = 14)
    png(filename, res = 150, height = 800, width = 1100)
    plot(p)
    dev.off()
