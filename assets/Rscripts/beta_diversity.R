...
# VENDORED from biobakery_workflows 3.2 (Rscripts/beta_diversity.R).
#
# One change, marked inline below: the univariate branch called `adonis()`,
# which is defunct in vegan 2.7.2 ("'adonis' is defunct. Use 'adonis2'
# instead."), so every univariate beta diversity task aborted. The pairwise and
# multivariate branches of this same script already call adonis2 and are
# untouched.
#
# The rename is not a drop-in: adonis() returned a list whose `$aov.tab` held
# the anova table, while adonis2() *is* the anova table (an anova.cca data
# frame). With by="terms" its first row is the single predictor, carrying the
# same `Pr(>F)` and `R2` the old `$aov.tab[1,]` did, so the numbers the rest of
# the branch plots are unchanged.
#
# biobakery_bootstrap.get_rscript() prefers this copy over the installed one.
# Delete this file once upstream moves to adonis2 and the pinned R stack moves.
...
if (current_args$pairwise) {
  ...
  adonis_rsq = vector()
  for (col in names(filtered_metadata)){
    # upstream has `adonis(...)` here and reads `$aov.tab[1,]`; see the note at
    # the top of this file
    adonis.univ = adonis2(as.formula(paste("bray ~", col)), data = filtered_metadata, by = "terms")
    adonis_pval[col] = adonis.univ[1, "Pr(>F)"]
    adonis_rsq[col] = adonis.univ[1, "R2"]
  }
