...
# VENDORED from biobakery_workflows 3.2 (Rscripts/mantel_test.R).
#
# One change, marked inline below: the script assumes every feature table has
# its samples as column names and transposes each one unconditionally. That
# holds for the abundance tables, but not for the count tables the data
# workflows write -- humann_feature_counts.tsv and
# humann_read_and_species_count_table.tsv have samples as *rows*, and
# create_feature_table.py copies them through without transposing. Transposing
# such a table leaves rows named for the count columns, the sample intersection
# with every other table is empty, and vegdist() returns an empty distance
# matrix, so ade4 aborts with "weights must be non-negative and not all zero"
# from bicenter.wt() before any test runs.
#
# The fix is the orientation check upstream's own beta_diversity.R already
# does, which is why the beta diversity tasks on the same tables succeed. Each
# table is now checked against the metadata sample names and only transposed
# when that is what makes its rows the samples. Tables already in the assumed
# orientation are untouched, so every existing result is unchanged.
#
# biobakery_bootstrap.get_rscript() prefers this copy over the installed one.
# Delete this file once upstream detects the orientation here too.
...
# Read the metadata up front so each data file's orientation can be checked
# against it. The metadata itself may be in either orientation -- that is what
# the check further down resolves -- so collect the sample names from both its
# rows and its columns, and allow for the special characters read.table turns
# into dots.
metadata <- data.frame(read.table(positional_args[1], header = TRUE, row.names = 1, sep="\t", comment.char = ""))
known_samples <- union(rownames(metadata), colnames(metadata))
known_samples <- union(known_samples, make.names(known_samples))
...
for (datafile in datafiles) {
    ...
    raw_data <- read.table(datafile, header = TRUE, row.names = 1, sep="\t", comment.char = "", quote = "")
    data <- t(raw_data)
    # upstream stops at the transpose above; see the note at the top of this
    # file. Only override it on positive evidence -- the transposed rows are
    # not samples and the untransposed ones are -- so a table in the assumed
    # orientation, or one whose samples are not named in the metadata at all,
    # is left exactly as upstream had it.
    if (length(intersect(rownames(data), known_samples)) < 1 &&
        length(intersect(colnames(data), known_samples)) > 0) {
...
# upstream reads the metadata here; it is now read before the loop above so the
# data files can be checked against it, and this is the same object
